...
class IndexController extends Controller
{
    public function clientes(){
        ...
        //Obtenemos clientes
        $clientes_morales = DB::table('clients')->orderBy('id', 'DESC')
        ->join('moral_people', 'moral_people.client_id', '=', 'clients.id')
        ->select('clients.id', 'moral_people.nombre', 'moral_people.emailFacturacion as email', 'moral_people.nombreFacturacion','moral_people.direccionFacturacion', 'moral_people.coloniaFacturacion', 'moral_people.numeroFacturacion', 'moral_people.created_at', 'clients.vetado')
        ->get();
        $clientes_fisicos = DB::table('clients')->take(850)->orderBy('id', 'DESC')
        ->join('physical_people', 'physical_people.client_id', '=', 'clients.id')
        ->select( 'clients.id', 'physical_people.nombre', 'physical_people.apellidoPaterno', 'physical_people.apellidoMaterno', 'physical_people.email', 'physical_people.nombreFacturacion', 'physical_people.direccionFacturacion', 'physical_people.coloniaFacturacion', 'physical_people.numeroFacturacion', 'physical_people.created_at', 'clients.vetado')
        ->get();
...
public function archivarUsuario($id){
 ...
 ///////////FUNCIONES API
    ...
    public function GetContratos(){
        ...
        $fechaHoy = Carbon::yesterday();
        $presupuestosHistorial = Budget::orderBy('fechaEvento', 'DESC')->take(250)->where('tipo', 'CONTRATO')->where('archivado', 0)->whereDate('fechaEvento', '<=', $fechaHoy)->get();
        $Presupuestos=[];
    ...
    public function GetContratosHistorial(){
        $budgets = Budget::orderBy('id', 'ASC')->where('tipo', 'CONTRATO')->where('archivado', '0')->where('categoriaEvento', '!=', 'nube')->get();

        $fechaHoy = Carbon::yesterday();
        $presupuestosHistorial = Budget::orderBy('fechaEvento', 'DESC')->where('tipo', 'CONTRATO')->where('archivado', 0)->whereDate('fechaEvento', '<=', $fechaHoy)->get();
        $Presupuestos=[];
...
///////////TERMINA FUNCIONES API
    ...
    public function presupuestos2(){
        ...
        $fechaHoy = Carbon::yesterday();
        $presupuestosHistorial = Budget::orderBy('id', 'DESC')->take(250)->where('tipo', 'CONTRATO')->where('archivado', 0)->whereDate('fechaEvento', '<=', $fechaHoy)->get();
        $Presupuestos=[];
...
public function creditosAtrasados(){
    ...
    $creditos = Budget::orderBy('id', 'DESC')->where('pagado', null)->where('tipo', 'CONTRATO')->where('archivado', 'FALSE')->where('fechaEvento', '!=', null)->orWhere('pagado','!=', true)->where('tipo', 'CONTRATO')->where('archivado', 'FALSE')->where('fechaEvento', '!=', null)->get();
    foreach ($creditos as $credito) {
        if(!is_null($credito->fechaEvento)){
